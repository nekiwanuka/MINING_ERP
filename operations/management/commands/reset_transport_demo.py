...
from operations.views import create_transport_receipt
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        pending_invoices = generate_transport_customer_invoices(pending_trip, user)

        issued_trip = self.create_trip(
            user=user,
            vehicle="UAX 118C",
            driver="John M.",
            origin="Entebbe Stores",
            destination="Jinja Quarry",
            distance_km=Decimal("125.00"),
            overall_charge=Decimal("640000.00"),
            customers=[
                {
                    "customer_name": "Jinja Aggregates",
                    "destination": "Jinja Quarry",
                    "cargo_description": "Conveyor belt rollers",
                    "package_type": "Pallets",
                    "loading_point": "Entebbe Stores",
                    "offloading_point": "Jinja Quarry",
                    "pieces": 2,
                    "cargo_charge": Decimal("640000.00"),
                }
            ],
            expenses=[
                (
                    ...
                    "Fuel estimate",
                    Decimal("120000.00"),
                    "Fuel before dispatch",
                )
            ],
        )
        issued_invoices = generate_transport_customer_invoices(issued_trip, user)
        self.issue_invoices(issued_invoices)

        paid_trip = self.create_trip(
            user=user,
            vehicle="UAY 804P",
            driver="Ivan S.",
            origin="Kampala Depot",
            destination="Masindi Site",
            distance_km=Decimal("310.00"),
            overall_charge=Decimal("980000.00"),
            customers=[
                {
                    "customer_name": "Masindi Stone Works",
                    "destination": "Masindi Site",
                    "cargo_description": "Crusher screens",
                    "package_type": "Crates",
                    "loading_point": "Kampala Depot",
                    "offloading_point": "Masindi Site",
                    "pieces": 3,
                    "cargo_charge": Decimal("980000.00"),
                }
            ],
            expenses=[
                (
                    ...
                    "Fuel paid",
                    Decimal("250000.00"),
                    "Fuel receipt recorded",
                )
            ],
        )
        paid_invoices = generate_transport_customer_invoices(paid_trip, user)
        self.issue_invoices(paid_invoices)
        for invoice in paid_invoices:
            receipt, _created = create_transport_receipt(invoice, user)
            invoice.payment_number = receipt.document_number
            invoice.paid_at = timezone.now()
            invoice.paid_by = user
            invoice.save(
                update_fields=["payment_number", "paid_at", "paid_by", "updated_at"]
            )
        paid_trip.status = TransportRecord.Status.IN_TRANSIT
        paid_trip.save(update_fields=["status", "updated_at"])
        ...
        self.issue_invoices(delivered_invoices)
        ...
        CommercialDocument.objects.create(
            ...
            title=f"Delivery note for {delivered_trip.transit_number}",
            transport=delivered_trip,
            business_reference=delivered_trip.transit_number,
            document_date=timezone.localdate(),
            client_name="Kasese Minerals, Kilembe Smelter",
            description=delivered_trip.customer_names_summary,
            notes=f"Route: {delivered_trip.origin} to {delivered_trip.destination}. Vehicle: {delivered_trip.vehicle}. Driver: {delivered_trip.driver}.",
            created_by=user,
        )

        self.stdout.write(
            self.style.SUCCESS(
                "Reset transport data and created seed trips for: "
                f"pending ({pending_trip.transport_number}, {len(pending_invoices)} invoice), "
                f"issued ({issued_trip.transport_number}, {len(issued_invoices)} invoice), "
                f"paid receipt ({paid_trip.transport_number}, {len(paid_invoices)} receipt), "
                f"in transit ({in_transit_trip.transport_number}), "
                f"and delivered ({delivered_trip.transport_number}, {len(delivered_invoices)} receipts + delivery note)."
            )
        )
    ...
    def create_trip(
        ...
        for customer in customers:
            record.customer_orders.create(**customer)
        self.create_expenses(record, expenses)
        return record

    def issue_invoices(self, invoices):
